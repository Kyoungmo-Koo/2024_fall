...
int NUM_OF_POSITIONS_RASTER = 8112;
int NUM_OF_POSITIONS_RADIAL = 8544;
int NUM_OF_BYTES_PER_CHUNK;
int NUM_OF_BYTES_PER_CHUNK_RASTER = 16224;
int NUM_OF_BYTES_PER_CHUNK_RADIAL = 17088;

uint32_t txData[8112];
uint32_t rxData[8112];
uint8_t StartByte;
...
void HAL_UART_TxCpltCallback (UART_HandleTypeDef *huart){
	if(count % 2 == 1){
		...
		HAL_SPI_Transmit_DMA(&hspi2, &rxData[4056], NUM_OF_BYTES_PER_CHUNK);
	}
}
...
void HAL_SPI_TxCpltCallback (SPI_HandleTypeDef *hspi){
	...
	if(count % 2 == 1){
		...
		HAL_SPI_Receive_DMA(&hspi2, &txData[4056], NUM_OF_BYTES_PER_CHUNK);
	}
}
