...
class raus_systsem_test():
    ...
    def control_exp(self):
        ...
        if mode_scan_use == "linear": 
            ...
            self._linear_scan_len_line_half = 0.0010
            ...
            for channel in range(frame.images.shape[-1]):
                ...
                if frame.format != FORMAT_CFM:
                    ...
                    if self.initial_check == 1 :
                        np.save(f"robot_automation_data/initial_check_bmode.npy", img_save_bmode_uint16[:, :, 0])
                        self.img = img_save_bmode_uint16[:, :, 0]
                        print("checkpoint-2")
                        self.initial_check  = 0
                    else: 
                        # np.save(f"{self._path_data_unique_folder}{self.idx_in_loop}_bmode.npy", img_save_bmode_uint16[:, :, 0])
                        np.save(f"robot_automation_data/{self.idx_in_loop2}_bmode.npy", img_save_bmode_uint16[:, :, 0])
                        self.img = img_save_bmode_uint16[:, :, 0]
                        self.idx_in_loop2 = self.idx_in_loop2 + 1
    ...
    def analyze_now(self):
        ...
        img = np.load(f"robot_automation_data/initial_check_bmode.npy")
        ...
        np.save(f"robot_automation_data/ee_{self.quant_count}.npy", T_ee_moved)
    ...
    def experiment(self, x_test, y_test):
        qs_home2 = self._robot.kinematics.inverse_nearest(self.move_in_cartesian(x_test, y_test), self.current_q)
    ...
    def main_data_platform(self): 
        ...
        ckpt_path = "circle_regressor_k10_20sets_perimage_split_mps.pt"
        ...
        T_ee = self._robot.kinematics.forward(self.current_q)
        np.save("robot_automation_data/ee_0.npy", T_ee)
        ...
        x_vals = np.random.uniform(-12.0, 12.0, 30)
        y_vals = np.random.uniform(-8.0, 8.0, 30)

        # print("checkpoint_1")
        # exit()

        for x_val, y_val in zip(x_vals, y_vals):
            print("finished:", self.quant_count)
            self.experiment(x_val, y_val)
