...
int NUM_OF_POSITIONS_RASTER = 8544;
int NUM_OF_POSITIONS_RADIAL = 8544;
int NUM_OF_BYTES_PER_CHUNK;
int NUM_OF_BYTES_PER_CHUNK_RASTER = 17088;
int NUM_OF_BYTES_PER_CHUNK_RADIAL = 17088;

uint32_t txData[5000];
uint32_t rxData[5000];
uint8_t StartByte;
...
void GenerateSinePositionData(uint32_t* positionDataArray, uint16_t arraySize){
	for(uint16_t i = 0; i < arraySize; i++){
		uint16_t x = (uint16_t)(65535 * sin(1.0 * M_PI * i / arraySize));
		uint16_t y = (uint16_t)(65535 * sin(1.0 * M_PI * i / arraySize));
...
void HAL_UART_TxCpltCallback (UART_HandleTypeDef *huart){
	...
	if(count % 2 == 1){
		//HAL_SPI_Receive_DMA(&hspi2, rxData, NUM_OF_BYTES_PER_CHUNK);
		LED_Toggle();
		HAL_UART_Receive_DMA(&huart1, txData, 2500 * 4);
		//LED_Toggle();
	}
	else if(count % 2 == 0){
		//HAL_SPI_Receive_DMA(&hspi2, &rxData[NUM_OF_BYTES_PER_CHUNK], NUM_OF_BYTES_PER_CHUNK);
		LED_Toggle();
		HAL_UART_Receive_DMA(&huart1, &txData[2500], 2500 * 4);
		//LED_Toggle();
	}
}
...
void HAL_SAI_TxCpltCallback (SAI_HandleTypeDef *hsai){
	...
	//HAL_Delay(1);
	if(count < COUNT + 1){
		HAL_UART_Transmit_DMA(&huart1, &rxData[2500], 1000 * 4);
		count = count + 1;
		substate = count % 4;
	}
}
...
void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai){
	...
	//HAL_Delay(1);
	if(count < COUNT + 1){
		HAL_UART_Transmit_DMA(&huart1, rxData, 1000 * 4);
		count = count + 1;
		substate = count % 4;
	}
}
...
/* Configure the peripherals common clocks */
  ...
  GenerateStepPositionData(10, txData, NUM_OF_POSITIONS);
  ...
  //HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t *)pwmData, 4);
  /* USER CODE END 2 */

  /* Infinite loop */
  /* USER CODE BEGIN WHILE */
  //GenerateSinePositionData(txData, NUM_OF_POSITIONS_RASTER);
    ...
    /* USER CODE END WHILE */
	  if(toggled == 0){
	 	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_SET){
	 		  SAI_TransmitReceive(NUM_OF_POSITIONS);
	 		  HAL_TIM_PWM_Start_DMA(&htim1, TIM_CHANNEL_1, (uint16_t *)pwmData, 8);
	 		  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);
	 		  toggled = 1;
	 	  }
	 	  }
