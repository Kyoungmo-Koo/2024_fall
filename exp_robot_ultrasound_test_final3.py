...
class raus_systsem_test():
    ...
    def main_data_platform(self): 
        ...
        # Move to center
        # qs_home_local = self._qs_home
        # self._ptp_exp.move_joint(qs_home_local, qd_limits=self._qd_limit_input)
        ...
        cross_section_image = np.load(f"{self._path_data_unique_folder}initial_check_bmode.npy")
        print("image collected, analysis start")
        print(cross_section_image.shape)
        
        column_mean = np.mean(cross_section_image, axis=0)
        num_columns = column_mean.shape[0]
        cutoff = int(num_columns * 0.5)
        sorted_indices = np.argsort(column_mean)[::-1]  # Descending
        top_indices = sorted_indices[:cutoff]
        avg_column_index = np.mean(top_indices)

        px_adjustment = avg_column_index - int(cross_section_image.shape[1] / 2)
        translation_distance = px_adjustment * 0.025 / int(cross_section_image.shape[1] / 2)
        print(translation_distance)

         # Step 1: Get the forward kinematics pose
        T_ee = self._robot.kinematics.forward(check_q)
        T_translate = np.eye(4)
        T_translate[:3, 3] = T_ee[:3, 1] * translation_distance * 4/5  # local Y axis

        T_ee_moved = T_ee @ T_translate

        # Step 3: Get the joint configuration for the new pose
        qs_home2 = self._robot.kinematics.inverse_nearest(T_ee_moved, check_q)

        # Move to the new configuration
        self._ptp_exp.move_joint(qs_home2, qd_limits=self._qd_limit_input)

        # rotate back
        rotate_back_q = qs_home2 + [0, 0, 0, 0, 0, np.pi / 2]
        self._ptp_exp.move_joint(rotate_back_q, qd_limits=self._qd_limit_input * 3)
