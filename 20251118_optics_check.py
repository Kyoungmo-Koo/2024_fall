...
class raus_systsem_test():
    ...
    def main_data_platform(self): 
        ...
        for finger_idx in range(5):
            ...
            if(finger_idx < 3):
                self._ptp_exp.move_joint(new_pip2_q, qd_limits= 3.0 * self._qd_limit_input)
            else:
                self._ptp_exp.move_joint(new_pip2_q, qd_limits= 5.0 * self._qd_limit_input)
            ...
            self._obj_ultrasound.save()
            if(finger_idx < 3):
                self._ptp_exp.move_joint(PIP_end_q, qd_limits= 2.0 * self._qd_limit_input )
            else:
                self._ptp_exp.move_joint(PIP_end_q, qd_limits= 3.0 * self._qd_limit_input )
        ...
        for finger_idx in range(5):
            ...
            if(finger_idx < 2):
                self._ptp_exp.move_joint(target_q2, qd_limits= 5.0 * self._qd_limit_input )
            else:
                self._ptp_exp.move_joint(target_q2, qd_limits= 3.0 * self._qd_limit_input)
            ...
            if(finger_idx < 2):
                self._ptp_exp.move_joint(MCP_end_q, qd_limits= 3.0 * self._qd_limit_input )
            else:
                self._ptp_exp.move_joint(MCP_end_q, qd_limits= 2.0 * self._qd_limit_input )
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        if pc.size > 0:
            ...
            np.save(fname, pc)
            print(f"✅ Saved {fname} ({pc.shape[0]} pts), ({pc.shape})")
        # time.sleep(1.0)
